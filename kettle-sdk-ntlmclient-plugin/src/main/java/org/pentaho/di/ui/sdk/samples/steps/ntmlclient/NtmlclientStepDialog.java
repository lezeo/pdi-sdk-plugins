package org.pentaho.di.ui.sdk.samples.steps.ntmlclient;
...
public class NtmlclientStepDialog<p> extends BaseStepDialog implements StepDialogInterface {
    private static Class<?> PKG = NtmlclientStepMeta.class; // for i18n purposes, needed by Translator2!!
    ...
    private NtmlclientStepMeta input;
    ...
    private ColumnInfo[] colinf, colinfoparams, colinfResultParams;
    ...
    private Label wlResultParams;
    private CTabItem wResultParamsTab;
    private TableView wResultParams;
    private Button wGetResultParams;

    public NtmlclientStepDialog( Shell parent, Object in, TransMeta transMeta, String sname ) {
        super( parent, (BaseStepMeta) in, transMeta, sname );
        input = ( NtmlclientStepMeta ) in;
    ...
    public String open() {
        ...
        wMethod.setItems( NtmlclientStepMeta.HTTP_METHODS );
        ...
        wApplicationType.setItems( NtmlclientStepMeta.APPLICATION_TYPES );
    ...
    private void addHeaderTab( ModifyListener lsMod ) {
        int margin = Const.MARGIN;
    ...
    private void addResultParamsTab( ModifyListener lsMod ) {
        int margin = Const.MARGIN;

        wResultParamsTab = new CTabItem( wTabFolder, SWT.NONE );
        wResultParamsTab.setText( BaseMessages.getString( PKG, "NtmlclientStepDialog.ResultParamsTab.TabTitle" ) );

        FormLayout resultParamsLayout = new FormLayout();
        resultParamsLayout.marginWidth = Const.FORM_MARGIN;
        resultParamsLayout.marginHeight = Const.FORM_MARGIN;

        Composite wResultParamsComp = new Composite( wTabFolder, SWT.NONE );
        wResultParamsComp.setLayout( resultParamsLayout );
        props.setLook( wResultParamsComp );

        // button
        wGetResultParams = new Button( wResultParamsComp, SWT.PUSH );
        wGetResultParams.setText(BaseMessages.getString(PKG, "System.Button.GetFields"));
        FormData fdGetResultParams = new FormData();
        fdGetResultParams.top = new FormAttachment( wlFields, Const.MARGIN );
        fdGetResultParams.right = new FormAttachment( 100, 0 );
        wGetResultParams.setLayoutData( fdGetResultParams );

        // column
        final int FieldsRows = input.getResultParamsName().length;
        colinfResultParams = new ColumnInfo[]{
                new ColumnInfo(
                        BaseMessages.getString(PKG, "NtmlclientStepDialog.ResultParamsTab.Property.Name"),
                        ColumnInfo.COLUMN_TYPE_TEXT,
                        false
                ),
                new ColumnInfo(
                        BaseMessages.getString(PKG, "NtmlclientStepDialog.ResultParamsTab.Property.TargetName"),
                        ColumnInfo.COLUMN_TYPE_TEXT,
                        false
                )
        };
        colinfResultParams[0].setUsingVariables( true );

        wResultParams = new TableView(
                transMeta,
                wResultParamsComp,
                SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI,
                colinfResultParams,
        ...
        FormData fdResultParams = new FormData();
        fdResultParams.left = new FormAttachment( 0, 0 );
        fdResultParams.top = new FormAttachment( 0, margin );
        fdResultParams.right = new FormAttachment( wGetResultParams, -margin );
        fdResultParams.bottom = new FormAttachment( 100, -margin );
        wResultParams.setLayoutData( fdResultParams );

        FormData fdResultParamsComp = new FormData();
        fdResultParamsComp.left = new FormAttachment( 0, 0 );
        fdResultParamsComp.top = new FormAttachment( 0, margin );
        fdResultParamsComp.right = new FormAttachment( 100, -margin );
        fdResultParamsComp.bottom = new FormAttachment( 100, 0 );
        wResultParamsComp.setLayoutData( fdResultParamsComp );
        wResultParamsComp.layout();

        wResultParamsTab.setControl( wResultParamsComp );
    }

    protected void setMethod() {
        boolean activateBody = NtmlclientStepMeta.isActiveBody( wMethod.getText() );
        boolean activateParams = NtmlclientStepMeta.isActiveParameters( wMethod.getText() );
    ...
    public void getData() {
        ...
        if ( input.getResultParamsName() != null ) {
            for ( int i = 0; i < input.getResultParamsName().length; i++ ) {
                TableItem item = wResultParams.table.getItem( i );
                if ( input.getResultParamsName()[i] != null ) {
                    item.setText( 1, input.getResultParamsName()[i] );
                }
                if ( input.getResultParamsTargetName()[i] != null ) {
                    item.setText( 2, input.getResultParamsTargetName()[i] );
                }
            }
        }

        wMethod.setText( Const.NVL( input.getMethod(), NtmlclientStepMeta.HTTP_METHOD_GET ) );
    ...
    private void ok() {
        ...
        int nrResultParams = wResultParams.nrNonEmpty();
        input.allocate( nrheaders, nrparams, nrmatrixparams, nrResultParams );
        ...
        for ( int i = 0; i < nrResultParams; i++ ) {
            TableItem item = wResultParams.getNonEmpty( i );
            input.getResultParamsName()[i] = item.getText( 1 );
            input.getResultParamsTargetName()[i] = item.getText( 2 );
        }
    ...
    private void getResultParams() {
        try ( CloseableHttpClient client = NtlmUtils.setHttpClient() ) {
            RequestBuilder requestBuilder = NtlmUtils.setRequestBuilder( wMethod.getText() );
            addQueryParams( requestBuilder );

            HttpClientContext context = HttpClientContext.create();
            addAuthentication( context );

            HttpUriRequest request = NtlmUtils.setRequest( requestBuilder, input.getUrl() );
            addHeaders( request );

            CloseableHttpResponse response = NtlmUtils.getResponse( client, request, context );
            overwriteResultParams( response );
        } catch (IOException | KettleException | ParserConfigurationException | SAXException e) {
            new ErrorDialog(
                    shell,
                    BaseMessages.getString( PKG, "RestDialog.FailedToGetHeaders.DialogTitle" ),
                    BaseMessages.getString( PKG, "RestDialog.FailedToGetHeaders.DialogMessage" ),
    ...
    private void addQueryParams( RequestBuilder builder ) {
        for ( int i = 0; i < wParameters.nrNonEmpty(); i++ ) {
            TableItem item = wParameters.getNonEmpty( i );
            String name = item.getText( 1 );
            String value = item.getText( 2 );
            Assert.notNull( name, "parameter field 不能为空" );
            Assert.notNull( value, "parameter name 不能为空" );
    ...
    private void addHeaders( HttpUriRequest request ) {
        for ( int i = 0; i < wFields.nrNonEmpty(); i++ ) {
            TableItem item = wFields.getNonEmpty( i );
            Assert.notNull( item.getText( 2 ), "header name 不能为空");
            String name = item.getText( 2 ).split(":")[0];
            String value = item.getText( 2 ).split(":")[1];
            request.addHeader( name, value );
        }
    }

    private void addAuthentication( HttpClientContext context ) {
        Assert.notNull( wHttpLogin.getText(), "用户名不能为空" );
        Assert.notNull( wHttpPassword.getText(), "密码不能为空" );
        String userName = wHttpLogin.getText();
        String domain = "";
        if ( userName.contains("/") ) {
            domain = userName.split("/")[0];
            userName = userName.split("/")[1];
        }
        context.setCredentialsProvider( NtlmUtils.getNtmlCredentials( userName, wHttpPassword.getText(), domain ) );
    }

    private void overwriteResultParams( CloseableHttpResponse response ) throws IOException, ParserConfigurationException, SAXException {
        wResultParams.clearAll();
    ...
    private void parseXml( String body ) throws ParserConfigurationException, IOException, SAXException {
        byte[] bytes = body.getBytes( StandardCharsets.UTF_8 );
        Document document = XmlUtils.xmlParse( bytes );

        NodeList nodeList = document.getElementsByTagName( "entry" );

        List<String> tabs = XmlUtils.getChildrenTab( nodeList );
        Assert.isTrue(!CollectionUtils.isEmpty( tabs ), "解析异常" );

        RowMetaInterface r = new RowMeta();
        tabs.forEach(it->{
            ValueMetaInterface v = new ValueMetaString( it );
    ...
    private void parseJson( String body ) {
        ...
        Map.Entry< String, Object > data = result.entrySet()
                .stream()
                .filter(it->it.getKey().equals("data"))
                .findFirst()
                .orElse(null);
        Assert.notNull( data, "result set is null" );

        // transform to RowMetaInterface
        RowMetaInterface r = new RowMeta();
        boolean isArray = JsonUtils.isJSONArray( data.getValue() );
        if ( isArray ) {
            JSONArray array = JSON.parseArray( data.getValue().toString());
            List<String> keys = JsonUtils.getKey( array );
            keys.forEach(it->{
                ValueMetaInterface v = new ValueMetaString( it );
                v.setOrigin( stepname );
                r.addValueMeta( v );
